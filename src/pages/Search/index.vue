...
export default {
  ...
  mounted() {
    // 再发请求之前带给服务器参数【searchParams参数发生变化由数值带给服务器】
    this.getData();
  },
  computed: {
  ...
  methods: {
    ...
    removeCategoryName() {
      // 设置为undefined不会带给服务器，设为空还会带给服务器【为了性能优化方面考虑】
      this.searchParams.categoryName = undefined;
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.getData();
      // 路径中的params参数不删除，跳转时要带上，只删除query参数
    ...
    attrInfo(item, attrValue) {
      // 参数格式整理好
      let props = `${item.attrId}:${attrValue}:${attrValue.attrName}`;
      // 数组去重
      // if语句里面只有一行代码，可以省略大括号
    ...
    // 排序操作
    changeOrder(flag) {
      // flag形参:代表用户点击的是 综合，还是价格
      let orginOrder = this.searchParams.order;
      let originsFlag = orginOrder.split(":")[0];
      let orginSort = orginOrder.split(":")[1];
      let newOrder = "";
      if (flag == originsFlag) {
        // 多次点击的是同一个按钮
        newOrder = `${originsFlag}:${orginSort == "desc" ? "asc" : "desc"}`;
      } else {
        // 点击的不是同一个按钮
        newOrder = `${flag}:${"desc"}`;
      }
