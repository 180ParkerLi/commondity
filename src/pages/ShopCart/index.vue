        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              class="itxt"
              :value="cart.skuNum"
              @change="handel('change', $event.target.value * 1, cart)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
              @click="handel('add', +1, cart)"
              >+</a
            >
          </li>
...
export default {
  ...
  methods: {
    ...
    handel: throttle(async function (type, disNum, cart) {
      /**
       * type:为了区分这三个元素
       * disNum: + 变化量（1） — 变化量（-1） 【0代表不变】 input最终的个数（并不是变化量）
       * cart:哪一个产品【身上又id】
       */
      switch (type) {
        //加号
      ...
      // 派发action
      try {
        await this.$store.dispatch("getAddOrUpdataShopCart", {
          skuId: cart.skuId,
          skuNum: disNum,
        });
        // 再一次获取服务器最新的数据进行展示
    ...
    updataChecked(cart, $event) {
      // 带给服务器的参数isChecked，不是布尔值，因该是 0 | 1
      try {
        let isChecked = event.target.checked ? "1" : "0";
        this.$store.dispatch("updataCheckedById", {
          skuId: cart.skuId,
          isChecked: isChecked,
        });
        this.getData();
